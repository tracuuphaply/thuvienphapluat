...
def _env(ten: str, mac_dinh: str = "") -> str:
    """Đọc biến môi trường, coi CHUỖI RỖNG là VẮNG MẶT.

    `os.getenv(ten, mac_dinh)` chỉ trả mặc định khi biến không tồn tại. Nhưng
    các nơi chạy thật đặt biến thành chuỗi RỖNG khi người vận hành chưa khai:
    GitHub Actions với `FOO: ${{ secrets.FOO }}` mà secret chưa có, Docker
    `-e FOO`, và dòng `FOO=` trong .env đều làm thế.

    Lỗi này đã xảy ra thật, ngày 26/08/2026: workflow truyền
    `OPENAI_API_BASE: ${{ secrets.OPENAI_API_BASE }}` cho một secret cố ý để
    trống (vì đã có mặc định trong code). Biến thành chuỗi rỗng, mặc định không
    bao giờ được dùng, URL thành "/chat/completions" và httpx từ chối. Các biến
    số còn hỏng nặng hơn: `int("")` ném ValueError chứ không chỉ sai giá trị.

    Đây là lý do hàm này thay cho os.getenv ở MỌI chỗ có mặc định khác rỗng.
    """
    return (os.getenv(ten) or "").strip() or mac_dinh
...
TVPL_RATE_LIMIT_SECONDS = float(_env("TVPL_RATE_LIMIT_SECONDS", "7"))
# Tải file .docx từ TVPL bằng Playwright (cần TVPL_USERNAME/PASSWORD)
TVPL_DOWNLOAD_ENABLED = _env("TVPL_DOWNLOAD_ENABLED", "true").lower() in (
    "true", "1", "yes",
)
...
TVPL_USE_CDP = _env("TVPL_USE_CDP", "true").lower() in ("true", "1", "yes")
...
TVPL_MAX_DOWNLOADS_PER_RUN = int(_env("TVPL_MAX_DOWNLOADS_PER_RUN", "40"))
# Số lần liên tiếp không thấy link tải thì coi như đã hết hạn mức và dừng batch
TVPL_MISSING_LINK_STREAK = int(_env("TVPL_MISSING_LINK_STREAK", "5"))
TVPL_CDP_PORT = int(_env("TVPL_CDP_PORT", "9222"))
...
MOJ_PAGE_SIZE = 100
MOJ_WINDOW_DAYS = int(_env("MOJ_WINDOW_DAYS", "30"))
...
MOJ_MAX_PAGES = int(_env("MOJ_MAX_PAGES", "40"))
...
GDRIVE_ROOT_FOLDER_NAME = _env("GDRIVE_ROOT_FOLDER_NAME", "Kho văn bản pháp luật")
...
LARK_DOMAIN = _env("LARK_DOMAIN", "open.larksuite.com")
# Mỗi văn bản được lưu trong một thư mục con riêng (chứa cả file TVPL và Bộ Tư pháp)
LARK_FOLDER_PER_DOC = _env("LARK_FOLDER_PER_DOC", "true").lower() in (
    "true", "1", "yes",
)

# Auto-delete local .docx files after upload to save disk space
AUTO_CLEANUP_LOCAL_FILES = _env("AUTO_CLEANUP_LOCAL_FILES", "true").lower() in ("true", "1", "yes")
...
CLOUD_DRIVE_PROVIDER = _env("CLOUD_DRIVE_PROVIDER", "lark").strip().lower()
...
def upload_closure_nodes() -> bool:
    ...
    """
    return _env("UPLOAD_CLOSURE_NODES", "true").strip().lower() in (
        "true", "1", "yes",
    )
...
def openai_api_base() -> str:
    ...
    return _env("OPENAI_API_BASE", "https://cheapkeyai.shop/v1")
...
def report_request_timeout() -> float:
    ...
    """
    return float(_env("REPORT_REQUEST_TIMEOUT", "600"))
...
def summary_max_tokens() -> int:
    ...
    return int(_env("SUMMARY_MAX_TOKENS", "3000"))
...
def form_classifier_max_tokens() -> int:
    ...
    return int(_env("FORM_CLASSIFIER_MAX_TOKENS", "600"))
...
def cam_nang_max_tokens() -> int:
    ...
    """
    return int(_env("CAM_NANG_MAX_TOKENS", "8000"))
...
def report_central_only() -> bool:
    ...
    """
    return _env("REPORT_CENTRAL_ONLY", "true").strip().lower() not in (
        "0", "false", "no", "off")
...
def embedding_batch_size() -> int:
    return int(_env("EMBEDDING_BATCH_SIZE", "64"))
...
CLOSURE_SEED_FLOOR = _env("CLOSURE_SEED_FLOOR", "2026-01-01")
...
# đăng chậm.
CRAWL_OVERLAP_DAYS = int(_env("CRAWL_OVERLAP_DAYS", "45"))
CLOSURE_ENABLED = _env("CLOSURE_ENABLED", "false").lower() in ("true", "1", "yes")
CLOSURE_MAX_FETCH_PER_RUN = int(_env("CLOSURE_MAX_FETCH_PER_RUN", "300"))
...
CLOSURE_HUB_INDEGREE = int(_env("CLOSURE_HUB_INDEGREE", "200"))
CLOSURE_MAX_NODES = int(_env("CLOSURE_MAX_NODES", "15000"))
...
CLOSURE_MAX_DEPTH = int(_env("CLOSURE_MAX_DEPTH", "0"))
...
MIN_FREE_DISK_GB = float(_env("MIN_FREE_DISK_GB", "5"))
...
IMPACT_SCORER_BASE_VERSION = _env("IMPACT_SCORER_VERSION", "v1.0.0")
...
MAX_REPORTS_PER_DAY = int(_env("MAX_REPORTS_PER_DAY", "5"))
REPORT_B_COOLDOWN_DAYS = int(_env("REPORT_B_COOLDOWN_DAYS", "7"))

# ──────────────────────────────────────────────
# Trang công khai
# ──────────────────────────────────────────────
PUBLIC_VAULT_BASE_URL = os.getenv("PUBLIC_VAULT_BASE_URL", "")
PUBLISH_VAULT_ENABLED = _env("PUBLISH_VAULT_ENABLED", "false").lower() in (
    "true", "1", "yes",
)
